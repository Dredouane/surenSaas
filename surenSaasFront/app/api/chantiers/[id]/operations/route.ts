...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/v1/chantiers/${params.id}/operations?org_id=${orgId}`, { credentials: 'include' });
...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}/api/v1/chantiers/${params.id}/operations?org_id=${orgId}`, {
