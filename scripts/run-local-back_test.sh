...
# Variables Gmail IMAP (mot de passe d'application - pas de OAuth)
export SUREN_GMAIL_RECEPTION_IMAP_ADRESS="${SUREN_GMAIL_RECEPTION_IMAP_ADRESS:-REDACTED_EMAIL}"
export SUREN_GMAIL_RECEPTION_IMAP_MDP="${SUREN_GMAIL_RECEPTION_IMAP_MDP:-}"
echo "  Gmail IMAP: adresse=${SUREN_GMAIL_RECEPTION_IMAP_ADRESS}, mot de passe: $([ -n "$SUREN_GMAIL_RECEPTION_IMAP_MDP" ] && echo '✅ défini' || echo '❌ MANQUANT')"

# Variables Hermès (proxy vers VPS)
export HERMES_API_URL="${HERMES_API_URL:-http://REDACTED/hermes-api/v1}"
export HERMES_API_KEY="${HERMES_AREV_TRAVAUX_API_KEY:-}"
echo "  Hermès API: url=${HERMES_API_URL}, key: $([ -n "$HERMES_API_KEY" ] && echo '✅ définie' || echo '❌ MANQUANTE')"
